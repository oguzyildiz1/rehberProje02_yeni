...
#deneme
def duzelt():
    dosya = open("telefon_rehberi2.txt","r+",encoding="utf-8")
    oku = dosya.read()
    cevir = ast.literal_eval(oku)
    isim_bulundu = False
    ...
    with open("telefon_rehberi2.txt","r",encoding="utf-8") as dosya:
        data = dosya.read()
        data_cevirilen = ast.literal_eval(data)
        print(data_cevirilen)
        print(type(data_cevirilen))
    """


"""
def duzelt():
    with open("telefon_rehberi1.txt","r",encoding="utf-8") as dosya:
        okunan = dosya.read()
    print("╔" + "═" * 24 + "╗")
    print("║   Düzenlenecek kişi:   ║")
    print("╚" + "═" * 24 + "╝")

    cevrilen = ast.literal_eval(okunan)
    aranan = input()
    dosya.close()

    with open("telefon_rehberi1.txt","w",encoding="utf-8") as dosya:
        for a in cevrilen:
            if a["adi"] == aranan:
                print(a["adi"], a["num"])
                yeniAd = input("Yeni isim: ")
                yeniNo = input("Yeni no: ")
                a["adi"] = yeniAd
                a["num"] = yeniNo
            # dosya.write(f"{str(a)},")
"""
